...
2. 在主服务器外的其它服务器部署一个服务端，然后
    - 可以打开`configs/server.json`，将`SForward->ShareKey`中的内容复制到主服务中节点列表处导入
    - 也可以留意程序运行时类似`build sforward share key : xxx`的输出，复制内容主服务中节点列表处导入
...
"SForward": {
    //分享key，自动生成，【无需修改】
    "ShareKey": "",
    //管理密钥，自动生成，第一个导入的信标服务可以修改本节点配置，【无需修改】
