...
1. `路由IP`：此时`路由IP`为真实局域网网段，`A端`访问`192.168.1.5`会访问到`B端`局域网下`192.168.1.5`的设备
2. `路由IP + 目标IP`：此时`路由IP`为虚假网段，可以是任意的，`目标IP`为真实局域网网段，`A端`访问`192.168.123.5`会访问到`B端`局域网下的`192.168.1.5`设备
