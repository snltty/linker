...
2. 在主服务器外的其它服务器部署一个服务端，然后
    ...
    - 将`SForward->ShareKey`(仅连接)或`SForward->ShareKeyManager`(能管理)复制，到主服务中节点列表处导入
    - 也可以留意程序运行时类似`build sforward share key : xxx`或`build sforward share key manager: xxx`的输出复制，到主服务中节点列表处导入
