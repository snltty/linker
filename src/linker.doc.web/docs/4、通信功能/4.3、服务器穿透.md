...
![Docusaurus Plushie](./img/reverse2.jpg)
![Docusaurus Plushie](./img/reverse3.jpg)
:::
...
2. 在主服务器外的其它服务器部署一个服务端，然后
    ...
    - 将`Reverse->ShareKey`(仅连接)或`Reverse->ShareKeyManager`(能管理)复制，到主服务中节点列表处导入
    - 也可以留意程序运行时类似`build reverse share key : xxx`或`build reverse share key manager: xxx`的输出复制，到主服务中节点列表处导入
3. 默认配置就是可用的，但是也可以对节点配置进行一些修改，在`configs/server.json->Reverse` 下修改配置
