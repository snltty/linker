...
{
    public sealed class DiscoveryProtocolMatcherMdns : IDiscoveryProtocolMatcher
    {
        public int GetQueryKeys(DiscoveryProtocolInfo protocol, ReadOnlySpan<byte> payload, ICollection<string> keys)
        {
            ...
            for (int i = 0; i < questionCount; i++)
            {
                ...
                added += AddMdnsKeys(name, type, dnsClass, keys);
            }
        ...
        private static int AddMdnsKeys(string name, ushort type, ushort dnsClass, ICollection<string> keys)
        {
            if (name.Length == 0)
            {
                return 0;
            ...
            DiscoveryProtocolKeyHelper.AddDistinct(keys, "mdns:" + name + ":*");
            DiscoveryProtocolKeyHelper.AddDistinct(keys, "mdns:" + name + ":" + type + ":" + normalizedClass);
