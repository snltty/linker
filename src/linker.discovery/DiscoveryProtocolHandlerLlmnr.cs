...
{
    public sealed class DiscoveryProtocolHandlerLlmnr : IDiscoveryProtocolHandler
    {
        ...
        {
            return DiscoveryProtocolHandlerDnsLike.GetQueryKeys(payload, keys, "llmnr");
        }

        public int GetResponseKeys(DiscoveryProtocolInfo protocol, ReadOnlySpan<byte> payload, ICollection<string> keys)
        {
            return DiscoveryProtocolHandlerDnsLike.GetResponseKeys(payload, keys, "llmnr");
        }
