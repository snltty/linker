<template>
    <el-dialog append-to=".app-wrap" :title="$t('flow.reverse')" class="options-center" top="1vh" destroy-on-close v-model="state.show" width="680">
        <div>
            <div class="head">
                <el-input v-trim v-model="state.page.Key" :placeholder="$t('flow.search.domain')"></el-input>
            </div>
            <el-table :data="state.list" stripe border size="small" width="100%" height="60vh" @sort-change="handleSort">
                <el-table-column prop="Key" :label="$t('flow.search.domain')" width="200"></el-table-column>
...
<script>
import { getReverseFlows } from '@/apis/flow';
    ...
    setup (props,{emit}) {
        ...
        const _getReverseFlows = ()=>{
            ...
            getReverseFlows({
                ...
                state.timer = setTimeout(_getReverseFlows,1000);
            }).catch((e)=>{
                state.timer = setTimeout(_getReverseFlows,1000);
            });
        }
