<template>
    <el-dialog :title="$t('status.flowServerForward')" class="options-center" top="1vh" destroy-on-close v-model="state.show" width="680">
        <div>
            <div class="head">
                <el-input v-trim v-model="state.page.Key" :placeholder="$t('status.flowSearchDomainPort')"></el-input>
            </div>
            <el-table :data="state.list" stripe border size="small" width="100%" height="60vh" @sort-change="handleSort">
                <el-table-column prop="Key" :label="$t('status.flowSearchDomainPort')" width="200"></el-table-column>
