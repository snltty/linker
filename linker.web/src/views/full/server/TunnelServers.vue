<template>
    <el-table :data="state.list" border size="small" width="100%" :height="`${state.height}px`" @cell-dblclick="handleCellClick">
        ...
        <el-table-column prop="ProtocolType" label="协议" width="100">
            <template #default="scope">
                <div>
                    <el-select disabled v-model="scope.row.ProtocolType" placeholder="Select" size="small" @change="handleEditBlur(scope.row, 'ProtocolType')">
        ...
        <el-table-column prop="Type" label="类别" width="100">
            <template #default="scope">
                <el-select disabled v-model="scope.row.Type" placeholder="Select" size="small" @change="handleEditBlur(scope.row, 'Type')">
...
export default {
    ...
    setup(props) {
        ...
        const state = reactive({
            list:globalData.value.config.Client.Tunnel.Servers.sort((a,b)=>a.Disabled - b.Disabled),
            types:[],
            height: computed(()=>globalData.value.height-90),
            interfaces:[]
        });
        watch(()=>globalData.value.config.Client.Tunnel.Servers,()=>{
            if(state.list.filter(c=>c['__editing']).length == 0){
                state.list = globalData.value.config.Client.Tunnel.Servers.sort((a,b)=>a.Disabled - b.Disabled);
            }
            state.interface = globalData.value.config.Client.Tunnel.Interface;
        });
