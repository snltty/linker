...
for %%c in (release,ReleaseMonitor) do (
...
for %%c in (release,ReleaseNetwork) do (
...
for %%r in (win-x64,win-arm64) do (
	...
	echo F|xcopy "public\\extends\\release\\%%r\\*" "public\\extends\\ReleaseMonitor\\%%r\\*"  /s /f /h /y
	...
	for %%c in (release,ReleaseMonitor) do (
		...
		echo F|xcopy "cmonitor.install.win\\dist\\*" "public\\extends\\%%c\\%%r\\*"  /s /f /h /y
	)
)
