...
{
    public sealed class TransportMsQuicSocket : ITunnelTransport
    {
        public string Name => "msquic-socket";
        ...
        public TransportMsQuicSocket(ITunnelAdapter tunnelAdapter)
        {
        ...
        private async Task<ITunnelConnection> ConnectForward(TunnelTransportInfo tunnelTransportInfo)
        {
            ...
            try
            {
                ...
                //绑定一个udp，用来给QUIC链接
                Socket localUdp = BindListen(remoteUdp, remoteEP);

                QuicConnection connection = connection = await QuicConnection.ConnectAsync(new QuicClientConnectionOptions
                {
                    RemoteEndPoint = new IPEndPoint(IPAddress.Loopback, (localUdp.LocalEndPoint as IPEndPoint).Port),
                ...
                return new TunnelConnectionMsQuicSocket
                {
                    LocalUdp = localUdp,
                    remoteUdp = remoteUdp,
                    Stream = quicStream,
        ...
        /// <returns></returns>
        private Socket BindListen(Socket remoteUdp, IPEndPoint remoteEP)
        {
        ...
        private async Task ListenConnectCallback(ListenAsyncToken token)
        {
            try
            {
                ...
                while (true)
                {
                    SocketReceiveFromResult result = await token.LocalUdp.ReceiveFromAsync(buffer, tempEp);
                    ...
                    if (token.Received == false)
                    {
                        ...
                        {
                            targetToken.RemoteEP = result.RemoteEndPoint as IPEndPoint;
                        }
                    }
                    token.Received = true;

                    //将quic来的消息，直接发送给远端
                    await token.RemoteUdp.SendToAsync(buffer.AsMemory(0, result.ReceivedBytes), token.RemoteEP);
                }
            ...
            finally
            {
                try
                {
                    token.LocalUdp.SafeClose();
                    token.RemoteUdp.SafeClose();
                }
                catch (Exception)
                {
                }
        ...
        private async Task ListenReceiveCallback(ListenAsyncToken token)
        {
                ...
                while (true)
                {
                    ...
                    if (token.Step == ListenStep.Auth)
                    {
                        ...
                        else
                        {
                            ...
                            await token.LocalUdp.SendToAsync(memory, result.RemoteEndPoint);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            token.RemoteUdp = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, System.Net.Sockets.ProtocolType.Udp);
                            token.RemoteUdp.WindowsUdpBug();
                        }
                        if (token.Received == false)
                        {
                            await token.RemoteUdp.SendToAsync(memory, token.RemoteEP);
                            token.Received = true;
                            token.RealRemoteEP = result.RemoteEndPoint as IPEndPoint;
                            stateDic.AddOrUpdate((token.RemoteUdp.LocalEndPoint as IPEndPoint).Port, token, (a, b) => token);

                            _ = ListenReceiveCallback(new ListenAsyncToken
                            {
                                LocalUdp = token.RemoteUdp,
                                Step = ListenStep.Forward,
                                RemoteEP = result.RemoteEndPoint as IPEndPoint,
                                RemoteUdp = token.LocalUdp,
                                RealRemoteEP = result.RemoteEndPoint as IPEndPoint,
                                Received = true,
                            });
                        }
                        else
            ...
            finally
            {
                try
                {
                    stateDic.TryRemove((token.RemoteUdp.LocalEndPoint as IPEndPoint).Port, out _);
        ...
        /// <returns></returns>
        private async Task OnUdpConnected(object _state, Socket localUdp, Socket remoteUdp, IPEndPoint remoteEP, QuicConnection quicConnection, QuicStream stream)
        {
            ...
            if (state.TransportName == Name)
            {
                try
                {
                    TunnelConnectionMsQuicSocket result = new TunnelConnectionMsQuicSocket
                    {
                        LocalUdp = localUdp,
                        remoteUdp = remoteUdp,
