<template>
    <div>
        <div class="flex">
            ...
            <el-switch v-model="data.running" :loading="data.loading" disabled @click="handleTuntap(data)"  size="small" inline-prompt active-text="O" inactive-text="F" >
               
            </el-switch>
        </div>
        <div>{{ data.LanIPs.join('、') }}</div>

        <el-dialog v-model="state.show" :close-on-click-modal="false" append-to=".app-wrap" title="设置虚拟网卡IP"
            width="400">
            <div>
