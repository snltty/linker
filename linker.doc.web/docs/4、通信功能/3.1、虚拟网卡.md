...
2. 在使用**A端**使用 `iptables -t nat -A POSTROUTING -o linker -s 192.168.1.0/24 -j MASQUERADE` 添加转发规则，将**B端**的**192.168.1.0/24**局域网IP转发到**linker**网卡
